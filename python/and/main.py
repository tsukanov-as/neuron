...
for i in range(1000): # подсчет вероятности срабатывания AND нейрона путем эксперимента
    ...
    for i in range(cassette_size):
        ...
        if total_spike == 2: # спайки пришли одновременно (функция AND)
            stress += 1
...
print("вероятность срабатывания AND нейрона из эксперимента:", total_fired / 1000)
print("вероятность срабатывания AND нейрона расчетная:", ap*bp)
